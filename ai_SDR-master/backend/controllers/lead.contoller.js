...
export const leadAddController = async (req, res) => {
    try {
        ...
        await createNotification(
            ...
            'lead',
            'New Lead Added',
            `${newLead.name} from ${newLead.company} has been added as a new lead`,
            { leadName: newLead.name, company: newLead.company, leadId: newLead._id }
        );
...
export const getAllMeetingsController = async (req, res) => {
    try {
        // Get all leads that are ready for meetings (meeting-ready leads)
        const meetings = await LeadModel.find({ 
            $or: [
                { readyToMeet: true },
                { status: "meeting" },
                { meetingDate: { $ne: null } }
